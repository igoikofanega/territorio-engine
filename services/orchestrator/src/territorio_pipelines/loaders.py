    ...
    for (cod, nombre), grupo in pend.groupby(["cod", "nombre"], sort=False):
        ...
        for i in range(0, len(filas), lote):
            ...
            try:
                ...
                # Un fallo que no es de cuota (respuesta malformada, corte de red) se
                # reintenta una vez; si persiste, el trozo cuenta como sin etiqueta y se
                # sigue. La espera entre peticiones ya la pone `llm.completar`.
                try:
                    marcas = extraccion.etiquetar(
                        client, cfg["modelo"], nombre, PROVINCIA_NOTICIAS_NOMBRE, trozo
                    )
